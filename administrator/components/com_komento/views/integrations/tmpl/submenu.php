	...
	<div class="m">
		...
		<div class="kmt-component-select">
			<?php
				...
				<?php // not ready yet. require styling
				if( false ) { ?>
				<table>
					<tr>
						<td><?php echo JText::_( 'COM_KOMENTO_SETTINGS_SELECT_COMPONENT' ) . ': ';?></td>
						<td>
							<?php foreach( $result as $item ) {
								$link = JURI::root() . 'administrator/index.php?option=com_komento&view=integrations&component=' . $item;
								$image = JURI::root() . 'administrator/components/com_komento/assets/images/components/' . $item . '.png';
								$class = $item == $component ? ' class="selected"' : ''; ?>
								<a<?php echo $class ; ?> href="<?php echo $link; ?>"><img src="<?php echo $image; ?>" width="32" /><?php echo JText::_( 'COM_KOMENTO_' . strtoupper( $item ) ); ?></a>
							<?php } ?>
						</td>
					</tr>
				</table>
