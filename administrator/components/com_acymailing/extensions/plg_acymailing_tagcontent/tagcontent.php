	...
	?>
		...
		<table width="100%" class="adminform">
			...
			<tr>
				...
				<br/><span id="pictsize" <?php if($pageInfo->pict != 'resized') echo 'style="display:none;"'; ?>><?php echo JText::_('CAPTCHA_WIDTH') ?>
	...
	?>
		...
		<table width="100%" class="adminform">
			...
			<tr>
				...
				<br/><span id="pictsizeauto" <?php if($this->params->get('default_pict','1') != 'resized') echo 'style="display:none;"'; ?> ><?php echo JText::_('CAPTCHA_WIDTH') ?>
		...
		if($tag->type != "title"){
			...
			if(ACYMAILING_J16 && !empty($article->images) && !empty($tag->pict)){
				...
				if(!empty($images->$pictVar)){
					...
					if(!empty($tag->link)) $contentText .= '<a href="'.$link.'" style="text-decoration:none" >';
					$contentText .= '<img style="'.$style.'" alt="" border="0" src="'.JURI::root().$images->$pictVar.'" />';
					if(!empty($tag->link)) $contentText .= '</a>';
				}
			}
